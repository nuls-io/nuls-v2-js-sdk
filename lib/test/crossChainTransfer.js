...
var toAddress = '8CPcA7kaXSHbWb3GHP7bd5hRLFu8RZv57rY9w';
...
async function transferTransaction(pri, pub, chainId, transferInfo) {
  //账户转出资产余额
  var balanceInfo = await getBalance(chainId, transferInfo.assetsChainId, transferInfo.assetsId, transferInfo.fromAddress);
  ...
  var mainNetBalanceInfo = await getBalance(chainId, 2, 1, transferInfo.fromAddress);
  ...
  if (chainId === transferInfo.assetsChainId && transferInfo.assetsId === 1) {
    ...
    localBalanceInfo = await getBalance(chainId, chainId, 1, transferInfo.fromAddress);
